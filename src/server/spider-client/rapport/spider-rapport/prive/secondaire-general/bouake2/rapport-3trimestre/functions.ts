...
const chp1_B_1_3_tableaux_statistiques_des_resultats = (StatutEleve: string) => {
  return new Promise(async (resolve, reject) => {
    try {
    let sql = `
          ...
          ORDER BY 1, 2,3,4;
          UNION ALL 
          SELECT Max(Niveaux.RefNiveau) AS RefNiveau, "Total" AS NiveauCourt, "Total Etab" AS ClasseCourt, "#E3E3E3" AS bg, Count(IIf([SEXE]=2,1,Null)) AS F1, Count(IIf([SEXE]=1,1,Null)) AS G1, [F1]+[G1] AS T1, Count(Elèves.RefElève) AS EffectTotal, Count(IIf([MOYG3] Is Not Null And [SEXE]=2,1,Null)) AS F2, Count(IIf([MOYG3] Is Not Null And [SEXE]=1,1,Null)) AS G2, [F2]+[G2] AS T2, Count(IIf([MOYG3] Is Null,Null,1)) AS EffectClasse, Count(IIf([MOYG3] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([MOYG3]>=10,1,Null)) AS Tranche1, IIf([EffectClasse]=0,0,Round([Tranche1]/[EffectClasse]*100,2)) AS Taux1, Count(IIf([MOYG3] Between 8.5 And 9.99,1,Null)) AS Tranche2, IIf([EffectClasse]=0,0,Round([Tranche2]/[EffectClasse]*100,2)) AS Taux2, Count(IIf([MOYG3] Between 0 And 8.49,1,Null)) AS Tranche3, IIf([EffectClasse]=0,0,Round([Tranche3]/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,0,Round(Sum([MOYG3])/[EffectClasse],2)) AS MoyClasse
          FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
          HAVING (((TypesClasses.filière)=1) AND ((Elèves.inscrit)=Yes) AND ((Elèves.StatutElève)${StatutEleve}))
          ORDER BY 1, 2, 3, 4;
    ...
    resolve(contentsArray);
      ...
      console.log(`err => chp1_B_1_3_tableaux_statistiques_des_resultats`);
      return reject(err);
    }
  });
};
...
,
      ...
      WHERE (((IIf([TypesClasses].[RefTypeClasse]=13,10,[TypesClasses].[RefTypeClasse]))<14) AND ((Notes.RangG3) Like '1e%') AND ((Filières.RefFilière)=1) AND ((TypesClasses.filière)=1))
...
const chp1_B_2_2_1_recapitulatif_des_deux_semestres_moyennes_annuelles = () => {
  return new Promise(async (resolve, reject) => {
    try {
      let sql= `
          ...
          ORDER BY Niveaux.RefNiveau, Classes.RefClasse, T_Notes.MOYG4 DESC;
          
      `;
      const sqlResult = await fetchFromMsAccess<IChp1_B_2_2_1[]>(sql, appCnx);
      if (sqlResult.length === 0) return resolve([
        {
          label: '',
          obj: {classeLong: '', pp: '', educ: '' },
      ...
      const contentsArray = sqlResult.map((item: IChp1_B_2_2_1, i: number) => {
        return {
          // c0:(i+1),
          c1: nv(item.NomComplet),
          c2: nv(item.Redoub),
          c3: nv(item.StatutEleve),
          c4: nv(item.MOYG1),
          c5: nv(item.MOYG2),
          c6: nv(item.MOYG3),
          c7: nv(item.MOYG4),
    ...
    resolve(result);
      ...
      console.log(`err => chp1_B_2_2_1_recapitulatif_des_deux_semestres_moyennes_annuelles`);
      return reject(err);
    }
  });
};

const chp1_B_2_2_2_tableaux_recapitulatif_des_resultats_de_fin_annee = (StatutEleve: string) => {
  return new Promise(async (resolve, reject) => {
    try {
    let sql = `
          SELECT Niveaux.RefNiveau, 
          Niveaux.NiveauCourt, Classes.ClasseCourt, 
          "#FFFF" AS bg, Count(IIf([SEXE]=2,1,Null)) AS F1, 
          Count(IIf([SEXE]=1,1,Null)) AS G1, [F1]+[G1] AS T1, 
          Count(Elèves.RefElève) AS EffectTotal, 
          Count(IIf([MOYG4] Is Not Null And [SEXE]=2,1,Null)) AS F2, 
          Count(IIf([MOYG4] Is Not Null And [SEXE]=1,1,Null)) AS G2,
          [F2]+[G2] AS T2, 
          ...
          ORDER BY 1, 2,3,4;
          UNION ALL 
          SELECT Max(Niveaux.RefNiveau) AS RefNiveau, "Total" AS NiveauCourt, "Total Etab" AS ClasseCourt, "#E3E3E3" AS bg, Count(IIf([SEXE]=2,1,Null)) AS F1, Count(IIf([SEXE]=1,1,Null)) AS G1, [F1]+[G1] AS T1, Count(Elèves.RefElève) AS EffectTotal, 
          Count(IIf([MOYG4] Is Not Null And [SEXE]=2,1,Null)) AS F2, 
          Count(IIf([MOYG4] Is Not Null And [SEXE]=1,1,Null)) AS G2, [F2]+[G2] AS T2, 
          Count(IIf([MOYG4] Is Null,Null,1)) AS EffectClasse, 
          Count(IIf([MOYG4] Is Null,1,Null)) AS EffectNonClasse, 
          Count(IIf([MOYG4]>=10,1,Null)) AS Tranche1, 
          IIf([EffectClasse]=0,0,Round([Tranche1]/[EffectClasse]*100,2)) AS Taux1, 
    ...
    resolve(contentsArray);
      ...
      console.log(`err => chp1_B_2_2_2_tableaux_recapitulatif_des_resultats_de_fin_annee`);
      return reject(err);
    }
  });
};

const chp1_B_2_2_4_tableaux_recapitulatif_des_resultats_de_fin_annee_eleves_affecte_et_non = () => {
  return new Promise(async (resolve, reject) => {
    try {
    let sql = `
            SELECT Niveaux.RefNiveau, Niveaux.NiveauCourt, 
            Classes.ClasseCourt, "#FFFF" AS bg, 
            Count(IIf([SEXE]=2,1,Null)) AS F1, 
            Count(IIf([SEXE]=1,1,Null)) AS G1, [F1]+[G1] AS T1, Count(Elèves.RefElève) AS EffectTotal, 
            Count(IIf([MOYG4] Is Not Null And [SEXE]=2,1,Null)) AS F2, Count(IIf([MOYG4] Is Not Null And [SEXE]=1,1,Null)) AS G2,
            [F2]+[G2] AS T2, Count(IIf([MOYG4] Is Null,Null,1)) AS EffectClasse, Count(IIf([MOYG4] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([MOYG4]>=10,1,Null)) AS Tranche1, IIf([EffectClasse]=0,0,Round([Tranche1]/[EffectClasse]*100,2)) AS Taux1, Count(IIf([MOYG4] Between 8.5 And 9.99,1,Null)) AS Tranche2, IIf([EffectClasse]=0,0,Round([Tranche2]/[EffectClasse]*100,2)) AS Taux2, Count(IIf([MOYG4] Between 0 And 8.49,1,Null)) AS Tranche3, IIf([EffectClasse]=0,0,Round([Tranche3]/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,0,Round(Sum([MOYG4])/[EffectClasse],2)) AS MoyClasse
            FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
            GROUP BY Niveaux.RefNiveau, Niveaux.NiveauCourt, Classes.ClasseCourt, TypesClasses.filière, Elèves.inscrit
            HAVING (((TypesClasses.filière)=1) AND ((Elèves.inscrit)=Yes))
            ORDER BY 1, 2,3,4;
            ...
            ORDER BY 1, 2,3,4;
            UNION ALL 
            SELECT Max(Niveaux.RefNiveau) AS RefNiveau, "Total" AS NiveauCourt, "Total Etab" AS ClasseCourt, "#E3E3E3" AS bg, Count(IIf([SEXE]=2,1,Null)) AS F1, Count(IIf([SEXE]=1,1,Null)) AS G1, [F1]+[G1] AS T1, Count(Elèves.RefElève) AS EffectTotal, Count(IIf([MOYG4] Is Not Null And [SEXE]=2,1,Null)) AS F2, Count(IIf([MOYG4] Is Not Null And [SEXE]=1,1,Null)) AS G2, [F2]+[G2] AS T2, Count(IIf([MOYG4] Is Null,Null,1)) AS EffectClasse, Count(IIf([MOYG4] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([MOYG4]>=10,1,Null)) AS Tranche1, IIf([EffectClasse]=0,0,Round([Tranche1]/[EffectClasse]*100,2)) AS Taux1, Count(IIf([MOYG4] Between 8.5 And 9.99,1,Null)) AS Tranche2, IIf([EffectClasse]=0,0,Round([Tranche2]/[EffectClasse]*100,2)) AS Taux2, Count(IIf([MOYG4] Between 0 And 8.49,1,Null)) AS Tranche3, IIf([EffectClasse]=0,0,Round([Tranche3]/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,0,Round(Sum([MOYG4])/[EffectClasse],2)) AS MoyClasse
            FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
            WHERE (((TypesClasses.filière)=1) AND ((Elèves.inscrit)=Yes))
            ORDER BY 1, 2, 3, 4;
              `;
    ...
    resolve(contentsArray);
      ...
      console.log(`err => chp1_B_2_2_4_tableaux_recapitulatif_des_resultats_de_fin_annee_eleves_affecte_et_non`);
      return reject(err);
    }
  });
};

const chp1_B_2_2_5_liste_major_classe_par_niveau_fin_annee = () => {
  return new Promise(async (resolve, reject) => {
    try {
      let sql = `SELECT IIf([TypesClasses].[RefTypeClasse]=13,10,[TypesClasses].[RefTypeClasse]) AS RefTypeClasse, 
      ...
      console.log(`err => chp1_B_2_2_5_liste_major_classe_par_niveau_fin_annee`);
      return reject(err);
    }
  });
};
...
const chp2_B_situation_des_effectifs_apres_le_conseil_des_classes = () => {
  return new Promise(async (resolve, reject) => {
    try {
    let sql = `
              SELECT Niveaux.RefNiveau, IIf(IsNull([Cycle]),Null,[cycle]) AS CycleX, 1 AS orderby, [NiveauCourt] & " " & [Série] AS NiveauSerie, "#FFFF" AS bg, Count(IIf([SEXE]=2,1,Null)) AS F1, Count(IIf([SEXE]=1,1,Null)) AS G1, [F1]+[G1] AS T1, Count(Elèves.RefElève) AS EffectTotal, Count(IIf([MOYG3] Is Not Null And [SEXE]=2,1,Null)) AS F2, Count(IIf([MOYG3] Is Not Null And [SEXE]=1,1,Null)) AS G2, [F2]+[G2] AS T2, Count(IIf([MOYG3] Is Null,Null,1)) AS EffectClasse, Count(IIf([MOYG3] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([SEXE]=2 And [Redoub]=True,1,Null)) AS F3, Count(IIf([SEXE]=1 And [Redoub]=True,1,Null)) AS G3, [F3]+[G3] AS T3, Count(IIf([SEXE]=2 And [MOYG3]>=10,1,Null)) AS F4, Count(IIf([SEXE]=1 And [MOYG3]>=10,1,Null)) AS G4, [F4]+[G4] AS T4, Count(IIf([MOYG3]>=10,1,Null)) AS Tranche1, IIf([EffectClasse]=0,0,Round([Tranche1]/[EffectClasse]*100,2)) AS Taux1, Count(IIf([MOYG3] Between 8.5 And 9.99,1,Null)) AS Tranche2, IIf([EffectClasse]=0,0,Round([Tranche2]/[EffectClasse]*100,2)) AS Taux2, Count(IIf([MOYG3] Between 0 And 8.49,1,Null)) AS Tranche3, IIf([EffectClasse]=0,0,Round([Tranche3]/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,0,Round(Sum([MOYG3])/[EffectClasse],2)) AS MoyClasse
              FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
              ...
              UNION ALL(
              SELECT First(Niveaux.RefNiveau) AS RefNiveau, IIf(IsNull([Cycle]),Null,[cycle]) AS CycleX, 2 AS orderby, "Total " & First([CycleX]) AS NiveauSerie, "#EBEBEB" AS bg, Count(IIf([SEXE]=2,1,Null)) AS F1, Count(IIf([SEXE]=1,1,Null)) AS G1, [F1]+[G1] AS T1, Count(Elèves.RefElève) AS EffectTotal, Count(IIf([MOYG3] Is Null Or [SEXE]=2,Null,1)) AS F2, Count(IIf([MOYG3] Is Null Or [SEXE]=1,Null,1)) AS G2, [F2]+[G2] AS T2, Count(IIf([MOYG3] Is Null,Null,1)) AS EffectClasse, Count(IIf([MOYG3] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([SEXE]=2 And [Redoub]=True,1,Null)) AS F3, Count(IIf([SEXE]=1 And [Redoub]=True,1,Null)) AS G3, [F3]+[G3] AS T3, Count(IIf([SEXE]=2 And [MOYG3]>=10,1,Null)) AS F4, Count(IIf([SEXE]=1 And [MOYG3]>=10,1,Null)) AS G4, [F4]+[G4] AS T4, Count(IIf([MOYG3]>=10,1,Null)) AS Tranche1, IIf([EffectClasse]=0,0,Round([Tranche1]/[EffectClasse]*100,2)) AS Taux1, Count(IIf([MOYG3] Between 8.5 And 9.99,1,Null)) AS Tranche2, IIf([EffectClasse]=0,0,Round([Tranche2]/[EffectClasse]*100,2)) AS Taux2, Count(IIf([MOYG3] Between 0 And 8.49,1,Null)) AS Tranche3, IIf([EffectClasse]=0,0,Round([Tranche3]/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,0,Round(Sum([MOYG3])/[EffectClasse],2)) AS MoyClasse
              FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
              ...
              UNION ALL (
              SELECT Last([Niveaux].[RefNiveau]) AS RefNiveau, "2nd Cycle" AS CycleX, 1 AS orderby, "Tle A" AS NiveauSerie, "#FFFF" AS bg, Count(IIf([SEXE]=2,1,Null)) AS F1, Count(IIf([SEXE]=1,1,Null)) AS G1, [F1]+[G1] AS T1, 0 AS EffectTotal, Count(IIf([MOYG3] Is Not Null And [SEXE]=2,1,Null)) AS F2, Count(IIf([MOYG3] Is Not Null And [SEXE]=1,1,Null)) AS G2, [F2]+[G2] AS T2, Count(IIf([MOYG3] Is Null,Null,1)) AS EffectClasse, Count(IIf([MOYG3] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([SEXE]=2 And [Redoub]=True,1,Null)) AS F3, Count(IIf([SEXE]=1 And [Redoub]=True,1,Null)) AS G3, [F3]+[G3] AS T3, Count(IIf([SEXE]=2 And [MOYG3]>=10,1,Null)) AS F4, Count(IIf([SEXE]=1 And [MOYG3]>=10,1,Null)) AS G4, [F4]+[G4] AS T4, Count(IIf([MOYG3]>=10,1,Null)) AS Tranche1, IIf([EffectClasse]=0,0,Round([Tranche1]/[EffectClasse]*100,2)) AS Taux1, Count(IIf([MOYG3] Between 8.5 And 9.99,1,Null)) AS Tranche2, IIf([EffectClasse]=0,0,Round([Tranche2]/[EffectClasse]*100,2)) AS Taux2, Count(IIf([MOYG3] Between 0 And 8.49,1,Null)) AS Tranche3, IIf([EffectClasse]=0,0,Round([Tranche3]/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,0,Round(Sum([MOYG3])/[EffectClasse],2)) AS MoyClasse
              FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
              ...
              UNION ALL(
              SELECT 8 AS RefNiveau, "Synthèse" AS CycleX, 3 AS orderby, "Total GENERAL" AS NiveauSerie, "#E3E3E3" AS bg, Count(IIf([SEXE]=2,1,Null)) AS F1, Count(IIf([SEXE]=1,1,Null)) AS G1, [F1]+[G1] AS T1, Count(Elèves.RefElève) AS EffectTotal, Count(IIf([MOYG3] Is Null Or [SEXE]=2,Null,1)) AS F2, Count(IIf([MOYG3] Is Null Or [SEXE]=1,Null,1)) AS G2, [F2]+[G2] AS T2, Count(IIf([MOYG3] Is Null,Null,1)) AS EffectClasse, Count(IIf([MOYG3] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([SEXE]=2 And [Redoub]=True,1,Null)) AS F3, Count(IIf([SEXE]=1 And [Redoub]=True,1,Null)) AS G3, [F3]+[G3] AS T3, Count(IIf([SEXE]=2 And [MOYG3]>=10,1,Null)) AS F4, Count(IIf([SEXE]=1 And [MOYG3]>=10,1,Null)) AS G4, [F4]+[G4] AS T4, Count(IIf([MOYG3]>=10,1,Null)) AS Tranche1, IIf([EffectClasse]=0,0,Round([Tranche1]/[EffectClasse]*100,2)) AS Taux1, Count(IIf([MOYG3] Between 8.5 And 9.99,1,Null)) AS Tranche2, IIf([EffectClasse]=0,0,Round([Tranche2]/[EffectClasse]*100,2)) AS Taux2, Count(IIf([MOYG3] Between 0 And 8.49,1,Null)) AS Tranche3, IIf([EffectClasse]=0,0,Round([Tranche3]/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,0,Round(Sum([MOYG3])/[EffectClasse],2)) AS MoyClasse
              FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
...
const chp4_D_handicaps = () => {
  return new Promise(async (resolve, reject) => {
    try {
      let sql = `
      SELECT Elèves.MatriculeNational, [NomElève] & " " & [PrénomElève] AS NomComplet, Classes.ClasseCourt, [MobilePère] & " \ " & [MobileMère] AS ContactsFamille, Format([Elèves].[DateNaiss],"Short Date") AS DateNaiss, tbl_cs_handicap.NatureHandicap, tbl_cs_handicap.OrigineHandicap
...
const rapport = (data: any): Promise<any> => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const chp1_B_2_2_1 = await chp1_B_2_2_1_recapitulatif_des_deux_semestres_moyennes_annuelles(); //Elèves affectés
      ...
      const chp1_B_1_3 = await chp1_B_1_3_tableaux_statistiques_des_resultats("=1"); //Elèves affectés
      const chp1_B_1_4 = await chp1_B_1_3_tableaux_statistiques_des_resultats("=2"); //Elèves non affectés
      const chp1_B_2_2_2 = await chp1_B_2_2_2_tableaux_recapitulatif_des_resultats_de_fin_annee("=1"); //Elèves affectés
      const chp1_B_2_2_3 = await chp1_B_2_2_2_tableaux_recapitulatif_des_resultats_de_fin_annee("=2"); //Elèves non affectés
      const chp1_B_2_2_4 = await chp1_B_2_2_4_tableaux_recapitulatif_des_resultats_de_fin_annee_eleves_affecte_et_non(); //Elèves et non affectés
      const chp1_B_2_2_5 = await chp1_B_2_2_5_liste_major_classe_par_niveau_fin_annee(); //Elèves et non affectés
      ...
      const result = {
        ...dataParams,
        name_report: "prive_secondairegeneral_bouake2_3trimestre",
      ...
      // console.log("🚀 ~ file: functions.ts:1482 ~ returnnewPromise ~ result:", result.chp1_B_2_2_1)
      resolve(result);
    } catch (err: any) {
      return reject(err);
    }
