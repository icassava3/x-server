...
import { addTGFRow } from "../rapport-2trimestre/functions";
...
const chp1_B_1_e_recapitulatif_des_resultats_des_eleves_affectes_et_non_par_niveau = (MoyG:string) => {
  return new Promise(async (resolve, reject) => {
    try {
      let sql = `SELECT Niveaux.RefNiveau, 1 AS orderby, Niveaux.NiveauCourt AS label, Niveaux.NiveauCourt, "#FFFF" AS bg, IIf([Elèves].[Sexe]=1,"G","F") AS genre, Count(Elèves.RefElève) AS EffectTotal, Count(IIf([${MoyG}] Is Null,Null,1)) AS EffectClasse, Count(IIf([${MoyG}] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([${MoyG}]>=10,1,Null)) AS Tranche1, IIf([EffectClasse]=0,0,Round([Tranche1]/[EffectClasse]*100,2)) AS Taux1, Count(IIf([${MoyG}] Between 8.5 And 9.99,1,Null)) AS Tranche2, IIf([EffectClasse]=0,0,Round([Tranche2]/[EffectClasse]*100,2)) AS Taux2, Count(IIf([${MoyG}] Between 0 And 8.49,1,Null)) AS Tranche3, IIf([EffectClasse]=0,0,Round([Tranche3]/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,Null,Round(Sum([${MoyG}])/[EffectClasse],2)) AS MoyClasse
                FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
                ...
                ORDER BY 1, 2;
                UNION ALL SELECT 8 AS RefNiveau, 2 AS orderby, "TOTAL" AS label, "TOTAL ETABL." AS NiveauCourt, "#E3E3E3" AS bg, "T" AS Genre, Count(Elèves.RefElève) AS EffectTotal, Count(IIf([${MoyG}] Is Null,Null,1)) AS EffectClasse, Count(IIf([${MoyG}] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([${MoyG}]>=10,1,Null)) AS Tranche1, IIf([EffectClasse]=0,0,Round([Tranche1]/[EffectClasse]*100,2)) AS Taux1, Count(IIf([${MoyG}] Between 8.5 And 9.99,1,Null)) AS Tranche2, IIf([EffectClasse]=0,0,Round([Tranche2]/[EffectClasse]*100,2)) AS Taux2, Count(IIf([${MoyG}] Between 0 And 8.49,1,Null)) AS Tranche3, IIf([EffectClasse]=0,0,Round([Tranche3]/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,Null,Round(Sum([${MoyG}])/[EffectClasse],2)) AS MoyClasse
                FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
                WHERE (((TypesClasses.filière)=1))
...
const chp1_B_2_liste_nominative = (StatutEleve: string) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      IIf(IsNull([Elèves].[LV2]),IIf([Classes].[LV2]="Aucune","",Left([Classes].[LV2],3)),Left([Elèves].[LV2],3)) AS Lang1, IIf([Classes].[LV2]="Aucune","",IIf([Classes].[LV2]="Mixte",Left([Elèves].[LV2],3),Left([Classes].[LV2],3))) AS Lang2, Elèves.LV2 AS Lang, T_Notes.MOYG3 AS MoyG2, Notes.RangG3, IIf([Elèves]![StatutElève]=1,"Affecté","Non Affecté") AS StatutEleve
...
const chp1_B_2_2_a_liste_nominative_fin_annee = (StatutEleve: string) => {
  return new Promise(async (resolve, reject) => {
    try {
      let sql = `
...
const rapport = (data: any): Promise<any> => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const chp1_B_1_2_b = await chp1_B_2_liste_nominative("=2")// Non affecté
      const chp1_B_1_3 = await chp1_B_3_liste_major_classe_par_niveau("MOYG3")

      const chp1_B_2_1_a = await chp1_B_1_1_a_tableaux_des_resultats_eleves_affectes_par_classe_par_niveau("MOYG4");
      const chp1_B_2_1_b = await chp1_B_1_1_b_tableaux_des_resultats_eleves_affectes_par_niveau("MOYG4");
      const chp1_B_2_1_c = await chp1_B_1_1_c_tableaux_des_resultats_eleves_non_affectes_par_classe_par_niveau("MOYG4");
      const chp1_B_2_1_d = await chp1_B_1_1_d_tableaux_des_resultats_eleves_non_affectes_par_niveau("MOYG4");
      const chp1_B_2_1_e = await chp1_B_1_e_recapitulatif_des_resultats_des_eleves_affectes_et_non_par_niveau("MOYG4");

      const chp1_B_2_2_a = await chp1_B_2_2_a_liste_nominative_fin_annee("=1")
      const chp1_B_2_2_b = await chp1_B_2_2_a_liste_nominative_fin_annee("=2")
      const chp1_B_2_3 = await chp1_B_3_liste_major_classe_par_niveau("MOYG4")


      const chp2_A = await chp2_D_effectif_par_niveau_avec_approche_genre()
      const chp2_B = await chp2_E_recap_effectif_classe_genre_aff_nonaffect()
      ...
      const result = {
        ...dataParams,
        name_report: "prive_secondairegeneral_yakro_3trimestre",
