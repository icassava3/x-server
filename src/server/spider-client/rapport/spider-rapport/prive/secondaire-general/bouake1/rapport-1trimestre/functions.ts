...
const chp1_B_1_1_6_tableaux_des_resultats_eleves_affectes_et_non_affectes_par_classe_par_niveau = () => {
  return new Promise(async (resolve, reject) => {
    try {
      let sql = `
      SELECT Niveaux.RefNiveau, Niveaux.NiveauCourt, Classes.ClasseCourt, Count(Elèves.RefElève) AS EffectTotal, Count(IIf([MOYG1] Is Null,Null,1)) AS EffectClasse, Count(IIf([MOYG1] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([MOYG1]>=10 And [SEXE]=1,1,Null)) AS G1, Count(IIf([MOYG1]>=10 And [SEXE]=2,1,Null)) AS F1, IIf([EffectClasse]=0,0,Round(([G1]+[F1])/[EffectClasse]*100,2)) AS Taux1, Count(IIf([MOYG1] Between 8.5 And 9.99 And [SEXE]=1,1,Null)) AS G2, Count(IIf([MOYG1] Between 8.5 And 9.99 And [SEXE]=2,1,Null)) AS F2, IIf([EffectClasse]=0,0,Round(([G2]+[F2])/[EffectClasse]*100,2)) AS Taux2, Count(IIf([MOYG1]<=8.5 And [SEXE]=1,1,Null)) AS G3, Count(IIf([MOYG1]<=8.5 And [SEXE]=2,1,Null)) AS F3, IIf([EffectClasse]=0,0,Round(([G3]+[F3])/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,0,Round(Sum([MOYG1])/[EffectClasse],2)) AS MoyClasse
      FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
      ...
      ORDER BY Classes.OrdreClasse;
      UNION ALL
      SELECT Niveaux.RefNiveau, Niveaux.NiveauCourt, "Total " & First([Niveaux].[NiveauCourt]) AS ClasseCourt, Count(Elèves.RefElève) AS EffectTotal, Count(IIf([MOYG1] Is Null,Null,1)) AS EffectClasse, Count(IIf([MOYG1] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([MOYG1]>=10 And [SEXE]=1,1,Null)) AS G1, Count(IIf([MOYG1]>=10 And [SEXE]=2,1,Null)) AS F1, IIf([EffectClasse]=0,0,Round(([G1]+[F1])/[EffectClasse]*100,2)) AS Taux1, Count(IIf([MOYG1] Between 8.5 And 9.99 And [SEXE]=1,1,Null)) AS G2, Count(IIf([MOYG1] Between 8.5 And 9.99 And [SEXE]=2,1,Null)) AS F2, IIf([EffectClasse]=0,0,Round(([G2]+[F2])/[EffectClasse]*100,2)) AS Taux2, Count(IIf(([MOYG1] Between 0 And 8.49) And [SEXE]=1,1,Null)) AS G3, Count(IIf(([MOYG1] Between 0 And 8.49) And [SEXE]=2,1,Null)) AS F3, IIf([EffectClasse]=0,0,Round(([G3]+[F3])/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,0,Round(Sum([MOYG1])/[EffectClasse],2)) AS MoyClasse
      FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
      ...
      UNION ALL (
      SELECT Max(Niveaux.RefNiveau) AS MaxDeRefNiveau, "" AS NiveauCourt, "Total Etabliss" AS ClasseCourt, Count(Elèves.RefElève) AS EffectTotal, Count(IIf([MOYG1] Is Null,Null,1)) AS EffectClasse, Count(IIf([MOYG1] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([MOYG1]>=10 And [SEXE]=1,1,Null)) AS G1, Count(IIf([MOYG1]>=10 And [SEXE]=2,1,Null)) AS F1, IIf([EffectClasse]=0,0,Round(([G1]+[F1])/[EffectClasse]*100,2)) AS Taux1, Count(IIf([MOYG1] Between 8.5 And 9.99 And [SEXE]=1,1,Null)) AS G2, Count(IIf([MOYG1] Between 8.5 And 9.99 And [SEXE]=2,1,Null)) AS F2, IIf([EffectClasse]=0,0,Round(([G2]+[F2])/[EffectClasse]*100,2)) AS Taux2, Count(IIf(([MOYG1] Between 0 And 8.49) And [SEXE]=1,1,Null)) AS G3, Count(IIf(([MOYG1] Between 0 And 8.49) And [SEXE]=2,1,Null)) AS F3, IIf([EffectClasse]=0,0,Round(([G3]+[F3])/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,0,Round(Sum([MOYG1])/[EffectClasse],2)) AS MoyClasse
      FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
...
const chp1_B_2_liste_major_classe_niveau = () => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      IIf([Elèves]![StatutElève]=1,"Affecté","Non Affecté") AS StatutEleve, Elèves.Décision AS NumDeciAffect, Notes.RangG1
      ...
      ORDER BY TypesClasses.RefTypeClasse, Classes.ClasseCourt, T_Notes.MOYG1 DESC , Niveaux.RefNiveau, Classes.ClasseCourt, [NomElève] & " " & [PrénomElève];  
      `;
...
const chp3_B_etat_du_personnel_administratif = () => {
  return new Promise(async (resolve, reject) => {
    try {
      let sql = `
      SELECT Personnel.RefPersonnel, [NomPers] & " " & [PrénomPers] AS NomComplet, 
      Diplomes.NomDiplome, Personnel.Sexe, Fonction.Fonction, Personnel.N°CNPS AS NumCnps
...
const rapport = (data: any): Promise<any> => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const result = {
        ...dataParams,
        name_report: "prive_secondairegeneral_bouake1_1trimestre",
      ...
      console.log("🚀 ~ file: functions.ts:1348 ~ returnnewPromise ~ result:", result.chp1_B_1_5)
      resolve(result);
    } catch (err: any) {
      return reject(err);
    }
