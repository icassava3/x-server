...
import {
  IChp1_B_1_b,
  IChp2_1, IChp2_2, IChp2_4, IChp3, IChp5_2, IChp6_2_1, IChp7_4_4, IChp_A_2_2, IChp_A_2_3,
} from "./interfaces";
const fs = require("fs");
...
const chp2_1_effectif_eleves_affect_et_non_affect = () => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      HAVING RefTypeClasse=5) AS _2ndA, 
      Count(IIf([TypesClasses].[RefTypeClasse] In (6),1,Null)) AS _2ndC, 
      Count(IIf([TypesClasses].[RefTypeClasse] In (7),1,Null)) AS _1ereA, 
      Count(IIf([TypesClasses].[RefTypeClasse] In (8),1,Null)) AS _1ereC, 
      Count(IIf([TypesClasses].[RefTypeClasse] In (9),1,Null)) AS _1ereD, 
...
const chp7_2_resultats_des_eleves_affectes = () => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      IIf([EffectClasse]<1,"",Round(Sum([MOYG1])/[EffectClasse],2)) AS MoyClasse
      FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
      ...
      Count(Elèves.RefElève) AS EffectTotal, 
      Count(IIf([MOYG1] Is Null,Null,1)) AS EffectClasse, Count(IIf([MOYG1] Is Null,1,Null)) AS EffectNonClasse, Count(IIf([MOYG1]>=10,1,Null)) AS Tranche1, IIf([EffectClasse]=0,0,Round([Tranche1]/[EffectClasse]*100,2)) AS Taux1, Count(IIf([MOYG1] Between 8.5 And 9.99,1,Null)) AS Tranche2, 
      IIf([EffectClasse]=0,0,Round([Tranche2]/[EffectClasse]*100,2)) AS Taux2, Count(IIf([MOYG1] Between 0 And 8.49,1,Null)) AS Tranche3, IIf([EffectClasse]=0,0,Round([Tranche3]/[EffectClasse]*100,2)) AS Taux3, IIf([EffectClasse]<1,"",Round(Sum([MOYG1])/[EffectClasse],2)) AS MoyClasse
      FROM ((Niveaux INNER JOIN (TypesClasses INNER JOIN Classes ON TypesClasses.RefTypeClasse = Classes.RefTypeClasse) ON Niveaux.RefNiveau = TypesClasses.Niveau) INNER JOIN T_Notes ON Classes.RefClasse = T_Notes.RefClasse) INNER JOIN Elèves ON T_Notes.RefElève = Elèves.RefElève
...
ORDER BY Classes.OrdreClasse, TypesClasses.RefTypeClasse, Niveaux.RefNiveau, Classes.RefClasse, [NomElève] & " " & [PrénomElève];
            `;
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      distinctRefTypeClasse.map(x => {
        ...
        const troisMajors = currentTypeClasseStudents.slice(0, 1) //je prends de 0 à 3 (non inclus)
      ...
      console.log(`err => chp7_5_liste_major_classe_par_niveau`);
      return reject(err);
    }
  });
};
...
const rapport = (data: any): Promise<any> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const result = {
        ...data,
        name_report: "prive_secondairegeneral_issia_1trimestre",
      ...
      // console.log("🚀 ~ file: functions.ts:1490 ~ returnnewPromise ~ result:", result.chp7_4_3)
      resolve(result);
    } catch (err: any) {
      return reject(err);
    }
