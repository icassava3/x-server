...
const pageGarde = (): Promise<any | {}> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const logo1 = path;
      const dataParams = { logo1: path, path: path };
...
const chp1_B_3_liste_major_classe_par_niveau = () => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      IIf([Elèves].[Sexe]=1,"M","F") AS Genre, IIf(IsNull([Elèves].[Nat]) Or [Elèves].[Nat]="70","",Left([Nationalités].[National],3)) AS Nationalite, IIf([Elèves]![Redoub]=True,"R","") AS Redoub, 
      IIf(IsNull([Elèves].[LV2]),IIf([Classes].[LV2]="Aucune","",Left([Classes].[LV2],3)),Left([Elèves].[LV2],3)) AS Lang2, 
      Elèves.LV2 AS Lang,
      IIf([Elèves]![StatutElève]=1,"Affecté","Non Affecté") AS StatutEleve, " " AS NumDeciAffect, Notes.RangG1
...
const rapport = (data: any): Promise<any> => {
  return new Promise(async (resolve, reject) => {
    ...
    const chp4_B_1 = await chp4_B_1_liste_du_personnel_administratif_et_encadrement();
      ...
      const result = {
        ...data,
        name_report: "prive_secondairegeneral_korhogo_1trimestre",
      ...
      console.log("🚀 ~ file: functions.ts:1307 ~ returnnewPromise ~ chp1_B_2_a:", result.chp1_A_4_a)
      resolve(result);
    } catch (err: any) {
      return reject(err);
    }
