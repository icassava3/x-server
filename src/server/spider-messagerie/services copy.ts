        ...
        try {
            ...
            switch (smsAccount.providerId) {
                ...
                case SMS_PROVIDER_EDIATTAH_ID:
                    ...
                    console.log("🚀 ~ returnnewPromise<{sms_messageId:string,sms_send_status:number}> ~ smsAccount:", smsAccount)
                    console.log("🚀 ~ returnnewPromise<{sms_messageId:string,sms_send_status:number}> ~ url3:", url3)
                    // const res3 = await axios.get(url3);
                    const response=  await fetch(url3);
                    console.log("responseeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee")
                    const res3 = await response.text()
                    console.log("🚀 ~ returnnewPromise<{sms_messageId:string,sms_send_status:number}> ~ res3:", res3)
                    // console.log("🚀 ~ file: services.ts:1063 ~ returnnewPromise ~ res3:", res3.data)

                    // res3.data = "OK: fe164bf7-975e-11ed-8f9a-0cc47a018caf"
                    const resSplit = res3.split(' ');
