...
import { FOCUS_APP_ID, MESSAGE_LOCKED_STATUS, MESSAGE_UNLOCKED_STATUS, PROF_EXPERT_APP_ID, SMS_PROVIDER_EDIATTAH_ID, SMS_PROVIDER_SPIDER_ID, SMS_PROVIDER_SYMTEL_ID, initialMiniBull2ndeATableBody, initialMiniBullLiteratureTableBody, initialMiniBullScienceTableBody, initialMiniBullTableBody, initialMiniBullTleA1TableBody } from './constants';
...
import { compteSmsCredit } from '../globalVariables';
import { PROF_EXPERT_SERVER_URL } from '../spider-prof-expert/constants';
 ...
 * @returns 
 */
const createOrUpdateSmsAccount = (io, data: IPayloadNewSmsAccount) => {
    return new Promise(async (resolve, reject) => {
        try {
            // await checkWarehouseActivatedAndAuthorizedHddSerialNumber();
            ...
            //Mise à jour du nombre de sms sur l'interface de x-server
...
const insererMessageGroupeBoiteEnvoi = (data: IPayloadMessageGroupe, io): Promise<any> => {
    return new Promise(async (resolve, reject) => {
        try {
            console.log("🚀 ~ insererMessageGroupeBoiteEnvoi ~ data:", JSON.stringify(data))
            const sessionId = uuid.v4();
            const { idEleves, messageTitle, messageContent, alertLevel, targetAppId, sendNow,teachersPhoneNumbers } = data;
            ...
            const todayDateTime = getTodayDateTime();
            let messages = []
            if (targetAppId === "FOCUS") {
                messages = parents.map(item => ([
                    anscol1,
                    codeetab,
                    sessionId,
                    item.idEleve,
            ...
            } else if(targetAppId === "PXP2021") {
                // idEleves: contient les numéros des professeurs qui doivent recevoir les notifications
                messages = teachersPhoneNumbers.map(item => ([
                    anscol1,
                    codeetab,
                    sessionId,
                    11111,
...
const envoyerMessage = (io, sessionIds: string[]): Promise<any> => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const appsMessages: ISendBoxMessages[] = [];//les messages a destination des parents d'eleves
            ...
            try {
                ...
                const BASE_URL = isFocusMessage ? FOCUS_ECOLE_BASE_URL : PROF_EXPERT_SERVER_URL
                console.log("🚀 ~ returnnewPromise ~ BASE_URL:", BASE_URL)
                const url = `${BASE_URL}/send-message`;
                const res = await fetchPublicRoute(url, { messages: appsMessages, anneeScolaire: anscol1, codeEtab: codeetab }) as any;
                ...
                const messageSendArray = messagesSent.map(item => ([
                    ...
                    item.messageId,
                    item.sessionId,
                    isFocusMessage ? item.studentId : item.phone,
                    isFocusMessage ? FOCUS_APP_ID : PROF_EXPERT_APP_ID,//targetAppId
                    item.phone,
                    item.messageTitle,
                    item.messageContent,
                    item.alertLevel,
                    1,//message locked
                    isFocusMessage ? item.fcmMessageId : item.pxpMessageId,
...
const envoyerSmsFromXServer = (io, providerId: number, sessionIds: string[] | null = null): Promise<any> => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            //Envois de crédit sms à l'interface web
            const etabCreditSms = compteSms.defaultAccount.creditSms
            io.emit("sms_envoyes", allSendBoxSms);
            io.emit("etab_credit_sms", etabCreditSms);
...
export const envoiSmsProvider = (smsAccount: ISmsAccount, phone: string, message: string): Promise<{ sms_messageId: string, sms_send_status: number }> => {
    return new Promise<{ sms_messageId: string, sms_send_status: number }>(async (resolve, reject) => {
        try {
            ...
            console.log("🚀 ~ returnnewPromise<{sms_messageId:string,sms_send_status:number}> ~ message:", message)
            ...
            switch (smsAccount.providerId) {
                ...
                case SMS_PROVIDER_EDIATTAH_ID:
                    // const url3 = `https://websms.spider-ci.com/api/api_http.php?username=${smsAccount.login}&password=${smsAccount.password}&sender=${smsAccount.sender}&to=${phoneWithCallingCode}&text=${message}&type=text`;
                    const url3 = `https://websms.spider-ci.com/api/api_http.php?username=${encodeURIComponent(smsAccount.login)}&password=${(smsAccount.password)}&sender=${encodeURIComponent(smsAccount.sender)}&to=${phoneWithCallingCode}&text=${encodeURIComponent(message)}&type=${smsMaxCharCount == 70 ? "unicode" : "text"}`;
                    console.log(":rocket: ~ returnnewPromise<{sms_messageId:string,sms_send_status:number}> ~ url3:", url3)
                    ...
                    // console.log(":rocket: ~ returnnewPromise<{sms_messageId:string,sms_send_status:number}> ~ res:", res3)
            ...
            console.log("🚀 ~ returnnewPromise<{sms_messageId:string,sms_send_status:number}> ~ error::::::::::", error)
            reject({
                sms_messageId: null,
                sms_send_status: 0,
            });
...
const recupMoyennesEleve = (periodeEval: number): Promise<IEleveMoyenne[]> => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const tNotesTech = periodeEval !== 2 ? await functions.fetchTNotesTechnique() : [];
            //    return resolve(tNotesTech)
            console.log("🚀 ~ file: services.ts:1755 ~ returnnewPromise ~ tNotesTech:", tNotesTech)
